// Dosya Yükleme Sistemleri (js/file-upload-managers.js)
...
const CryptoFileManager = {
  ...
  updateTableFromCSV(data) {
    ...
    data.forEach(coin => {
      ...
      let priceStr = coin['Fiyat'].replace(/\$/g, '');
      
      // Türkçe formatı düzelt (107.935,8 → 107935.8)
  ...
  hideUploadButton(type) {
    let uploadSection;
    
    if (type === 'crypto') {
      uploadSection = document.querySelector('#cryptoFile').parentElement;
    } else if (type === 'commodity') {
      uploadSection = document.querySelector('#commodityFile').parentElement;
    } else if (type === 'endeks') {
      uploadSection = document.querySelector('#endeksFile').parentElement;
    }
...
const CommodityFileManager = {
  ...
  updateTableFromExcel(headers, rows) {
    ...
    // Yükleme butonunu gizle
    this.hideUploadButton('commodity');
  ...
  // Upload butonunu gizle
  hideUploadButton(type) {
    const uploadSection = document.querySelector(`#${type}File`).parentElement;
    if (uploadSection) {
      uploadSection.style.display = 'none';
    }
  }
